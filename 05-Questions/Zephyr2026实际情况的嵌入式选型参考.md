...
- Zephyr 已从"小众 RTOS"转变为**行业基础设施**，背后有 Intel、Nordic、NXP、ST、TI、Meta 等公司长期投入，主线已有 **750+ 官方板级配置**。
...
- TI 维护有自己的 downstream 仓库 simplelink‑zephyr，用于补充更多设备支持。

### ● 支持程度

- TI 自 2016 年起即参与 Zephyr 项目，官方明确将 Zephyr 作为其 MCU 的重要 RTOS 选项。
...
- 主线支持 **ESP32、ESP32‑C2/C3/C6、ESP32‑S2/S3、ESP32‑H2** 等多代芯片。
- 覆盖度在 Wi‑Fi/BT 厂商中**最全面**。

### ● 支持程度

- Espressif 官方维护 Zephyr 支持页面，提供设备支持矩阵、外设支持表、最佳版本建议等。
...
Zephyr 主线确实支持 **ARM Cortex‑M、RISC‑V、Xtensa、ARC、x86** 等架构。  
...
1. **SoC 支持（soc/）**：必须有该芯片系列的 SoC 定义（寄存器、时钟树、中断控制器等）。
2. **DTS（DeviceTree）描述**：必须有完整的 DTS/DT bindings。
3. **驱动支持（drivers/）**：外设驱动必须在 Zephyr 中存在。
4. **板级配置（boards/）**：必须有具体开发板的 board 配置。
5. **启动代码（boot/）**：必须有适配该芯片的启动流程。
