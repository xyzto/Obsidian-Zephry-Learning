...
`struct k_mutex` 记录持有者线程指针（Owner）和等待队列。当高优先级线程尝试 `k_mutex_lock` 而锁已被低优先级线程持有时，内核调用 `z_thread_priority_set()` 临时将 Owner 提升至高优先级线程的优先级，防止中优先级线程抢占 Owner 导致高优先级线程无限等待（优先级翻转）。Owner 释放锁后优先级恢复原值，高优先级线程立即获得 CPU。这是 Mutex 与信号量的核心区别：信号量无 Owner 概念，无法实现优先级继承。
...
→ 源码路径：`kernel/mutex.c`
→ 关键函数：`z_impl_k_mutex_lock()`、`z_thread_priority_set()`、`z_impl_k_mutex_unlock()`
...
Mutex 的本质是"带所有权的信号量"加上"优先级继承算法"，能动态调整持有锁线程的优先级，确保高优先级任务的实时响应边界不被中优先级任务破坏。换成 STM32F103ZE 实板，Mutex 机制与 QEMU 完全一致，无迁移成本。
