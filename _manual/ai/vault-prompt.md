...
### 第二步：检查 Inbox
读取 `00-Inbox/` 文件列表。若文件数 ≥ 5，在汇报时主动提醒用户清理，列出文件名。

### 第三步：汇报就位
格式（不超过 5 行）：
```
✅ 已就位（来源：state.md）
QEMU 进度：已完成第一层 10/10，第二层 4/6，下一个：15 设备树覆盖
Inbox：[N 个文件，⚠️ 建议清理 / 正常]
等待指令。
```
...
- 若有，且该问题已在本次实验中得到解答 → 更新状态为 `#已解决`，填写 `## 结论` 字段
